...
"Including some additions and corrections by zelhat!
...
"zelhar edit: I want a block to start after a blank line!
...
"Edit - trying to make it work better (zelhar)
"syn region markdownCode matchgroup=markdownCodeDelimiter start="`\ze\([^`]\)" end="\([^`]\)\zs`" keepend contains=markdownLineStart
"syn region markdownCode matchgroup=markdownCodeDelimiter start="`` \=ze\([^`]\)" end="\([^`]\)\zs \=``" keepend contains=markdownLineStart
syn region markdownGHCodeBlock matchgroup=markdownCodeDelimiter start="^\s*\n```\n" end="^```\n\s*\n" contained  keepend
" I copied this from OLDmarkdown.vim because the '*,**,***' syntax seems broken
" here probably coliding with list syntax.

syn region htmlItalic  start=/\\\@<!\(^\|\A\)\@=\*\@<!\*\*\@!\S/    end=/\S\\\@<!\*\@<!\*\*\@!\($\|\A\)\@=/    contains=htmlBold,@Spell
syn region htmlBold    start=/\\\@<!\(^\|\A\)\@=\*\@<!\*\*\*\@!\S/  end=/\S\\\@<!\*\@<!\*\*\*\@!\($\|\A\)\@=/  contains=@Spell,htmlBoldItalic
syn region htmlBoldItalic start=/\\\@<!\(^\|\A\)\@=\*\@<!\*\*\*\*\@!\S/  end=/\S\\\@<!\*\@<!\*\*\*\*\@!\($\|\A\)\@=/  contains=@Spell,markdownLineStart
